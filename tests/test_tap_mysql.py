...
class TestTypeMapping(unittest.TestCase):
    ...
    def get_metadata_for_column(self, colName):
        return next(
            md for md in self.metadata if md["breadcrumb"] == ("properties", colName)
        )["metadata"]
    ...
    def test_tinyint(self):
        self.assertEqual(
            ...
            Schema(
                ["null", "integer"], inclusion="available", minimum=-128, maximum=127
            ),
        )
        self.assertEqual(
    ...
    def test_float(self):
        self.assertEqual(
            self.schema.properties["c_float"],
    ...
    def test_double(self):
        self.assertEqual(
            self.schema.properties["c_double"],
            Schema(["null", "number"], inclusion="available"),
        )
        self.assertEqual(
...
class TestSchemaMessages(unittest.TestCase):
    def runTest(self):
        ...
        test_utils.set_replication_method_and_key(
            catalog.streams[0], "FULL_TABLE", None
        )
...
class TestStreamVersionFullTable(unittest.TestCase):
    ...
    def test_with_no_initial_full_table_complete_in_state(self):
        ...
        self.assertFalse(
            "version" in state["bookmarks"]["tap_mysql_test-full_table"].keys()
        )
        self.assertTrue(
            state["bookmarks"]["tap_mysql_test-full_table"][
    ...
    def test_with_initial_full_table_complete_in_state(self):
        ...
        state = {
            "bookmarks": {
                "tap_mysql_test-full_table": {"initial_full_table_complete": True}
            }
        }
...
class TestIncrementalReplication(unittest.TestCase):
    ...
    def test_change_replication_key(self):
        ...
        self.assertEqual(
            state["bookmarks"]["tap_mysql_test-incremental"]["replication_key"], "val"
        )
        self.assertEqual(
...
class TestBinlogReplication(unittest.TestCase):
    ...
    def setUp(self):
        ...
        for stream in self.catalog.streams:
            ...
            self.state = singer.write_bookmark(
                self.state, stream.tap_stream_id, "log_pos", log_pos
            )
    ...
    def test_initial_full_table(self):
        ...
        activate_version_message_1 = list(
            filter(
                lambda m: isinstance(m, singer.ActivateVersionMessage)
                and m.stream == "tap_mysql_test-binlog_1",
                SINGER_MESSAGES,
            )
        )[0]
        ...
        activate_version_message_2 = list(
            filter(
                lambda m: isinstance(m, singer.ActivateVersionMessage)
                and m.stream == "tap_mysql_test-binlog_2",
                SINGER_MESSAGES,
            )
        )[0]

        self.assertIsNotNone(
            singer.get_bookmark(self.state, "tap_mysql_test-binlog_1", "log_file")
        )
        self.assertIsNotNone(
            singer.get_bookmark(self.state, "tap_mysql_test-binlog_1", "log_pos")
        )

        self.assertIsNotNone(
            singer.get_bookmark(self.state, "tap_mysql_test-binlog_2", "log_file")
        )
        self.assertIsNotNone(
            singer.get_bookmark(self.state, "tap_mysql_test-binlog_2", "log_pos")
        )
        ...
        for schema_msg in schema_messages:
            for prop, val in schema_msg.schema["properties"].items():
                self.assertIn(
                    "type",
                    val,
                    f'property "{prop}" has no "type" in stream "{schema_msg.stream}"',
                )
    ...
    def test_binlog_stream_with_alteration(self):
        ...
        with connect_with_backoff(self.conn) as open_conn:
            with open_conn.cursor() as cursor:
                ...
                cursor.execute(
                    "INSERT INTO binlog_1 (id, updated, is_cancelled) VALUES (3, '2017-09-21', false)"
                )
                cursor.execute(
                    "INSERT INTO binlog_2 (id, updated) VALUES (3, '2017-12-10')"
                )
                cursor.execute(
                    "ALTER TABLE binlog_1 change column updated date_updated datetime;"
                )
                cursor.execute(
                    "UPDATE binlog_1 set date_updated='2018-06-18' WHERE id = 3"
                )
    ...
    def test_do_not_discover_key_properties_for_view(self):
        ...
        for c in catalog.streams:
            primary_keys[c.table] = (
                singer.metadata.to_map(c.metadata)
                .get((), {})
...
class TestEscaping(unittest.TestCase):
    def setUp(self):
        ...
        test_utils.set_replication_method_and_key(
            self.catalog.streams[0], "FULL_TABLE", None
        )
    ...
    def setUp(self):
        ...
        with connect_with_backoff(self.conn) as open_conn:
            with open_conn.cursor() as cursor:
                ...
                cursor.execute(
                    'INSERT INTO json_table (val) VALUES ( \'{"a": 10, "b": "c"}\')'
                )
...
class TestSupportedPK(unittest.TestCase):
    ...
    def test_sync_messages_are_correct(self):
        ...
        self.catalog.streams[0] = test_utils.set_replication_method_and_key(
            self.catalog.streams[0], "LOG_BASED", None
        )
        ...
        state_message = list(
            filter(lambda m: isinstance(m, singer.StateMessage), SINGER_MESSAGES)
        )[-1]
        ...
        tap_mysql.do_sync(
            self.conn, test_utils.get_db_config(), self.catalog, state_message.value
        )
...
class TestSessionSqls(unittest.TestCase):
    ...
    def test_open_connections_with_default_session_sqls(self):
        ...
        self.assertEqual(
            self.executed_queries, tap_mysql.connection.DEFAULT_SESSION_SQLS
        )
