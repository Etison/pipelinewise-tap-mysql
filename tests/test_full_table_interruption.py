...
def singer_write_message_no_table_2(message):
    ...
    if (
        isinstance(message, singer.RecordMessage)
        and message.stream == "tap_mysql_test-table_2"
    ):
...
class BinlogInterruption(unittest.TestCase):
    ...
    def test_table_2_interrupted(self):
        ...
        try:
            tap_mysql.do_sync(
                self.conn, test_utils.get_db_config(), self.catalog, state
            )
